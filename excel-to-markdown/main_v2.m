...
for i = 1:row
    for j = 1:col
        out{i+(i>1),2*j} = set_string(TXT{i,j},TXT_length(i,j), max_col_length(j));
    end
end

%% 添加第二行的对齐方式
for j = 1:col
    out{2, 2*j} = set_align_type(align_type, max_col_length(j));
end
%% 写入txt文件
fp = fopen('.\output\Markdown_table.txt','w');

for i = 1:row+1
    table = [out{i,1:2*col+1}];
    fprintf(fp,'%s\n',table);
end
fclose(fp);
